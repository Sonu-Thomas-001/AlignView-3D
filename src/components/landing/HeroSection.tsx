...
const STAGE_DESCRIPTIONS = [
  { stage: 1, text: 'Stage 01: Initial Anterior Crowding & Rotation', progress: 3 },
  { stage: 8, text: 'Stage 08: De-rotating Lateral & Central Incisors', progress: 25 },
  { stage: 16, text: 'Stage 16: Transverse Premolar Arch Expansion', progress: 50 },
  { stage: 24, text: 'Stage 24: Restoring Canine Occlusal Guidance', progress: 75 },
  { stage: 32, text: 'Stage 32: Ideal Class I Occlusion Achieved', progress: 100 },
];

export const HeroSection: React.FC = () => {
  const [currentStageIdx, setCurrentStageIdx] = useState(1); // 0 to 4
  const [isPlaying, setIsPlaying] = useState(true);

  useEffect(() => {
    if (!isPlaying) return;
    const interval = setInterval(() => {
      setCurrentStageIdx((prev) => (prev + 1) % STAGE_DESCRIPTIONS.length);
    }, 2800);
    return () => clearInterval(interval);
  }, [isPlaying]);

  const activeStage = STAGE_DESCRIPTIONS[currentStageIdx];

  return (
    <section className="relative pt-32 pb-24 sm:pt-40 sm:pb-32 overflow-hidden bg-gradient-to-b from-blue-50/70 via-slate-50 to-white">
      {/* Dynamic Animated Ambient Glow Orbs */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] sm:w-[950px] h-[450px] bg-gradient-to-tr from-blue-400/25 via-sky-300/25 to-purple-400/15 rounded-full blur-3xl pointer-events-none -z-10 animate-pulse-glow" />
      <div className="absolute top-1/3 left-10 w-[350px] h-[350px] bg-blue-500/15 rounded-full blur-3xl pointer-events-none -z-10 animate-float-slow" />
      <div className="absolute top-1/2 right-10 w-[300px] h-[300px] bg-purple-500/15 rounded-full blur-3xl pointer-events-none -z-10 animate-float-reverse" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        
        {/* Top Product Badge with Glow Animation */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-100/90 border border-blue-200 text-blue-800 text-xs font-semibold shadow-sm mb-6 animate-in fade-in slide-in-from-bottom-3 duration-500 hover:scale-105 transition-transform cursor-default">
        ...
        {/* Hero Main Headline with Dynamic Gradient Shimmer */}
        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-slate-900 tracking-tight leading-[1.12] max-w-5xl mx-auto">
          ...
          <span className="bg-gradient-to-r from-blue-600 via-sky-400 to-indigo-600 bg-[length:200%_auto] bg-clip-text text-transparent animate-shimmer inline-block">
            at the Speed of Web.
          </span>
        </h1>
        ...
        <div className="mt-8 sm:mt-10 flex flex-wrap items-center justify-center gap-3 sm:gap-4">
          <Link
            href="/studio"
            className="group relative px-8 py-4 text-sm sm:text-base font-bold text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-full transition-all shadow-xl shadow-blue-500/30 hover:shadow-2xl hover:shadow-blue-500/45 flex items-center gap-2.5 transform hover:-translate-y-1 overflow-hidden"
          >
            {/* Shimmer button light */}
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-4 sm:gap-8 text-xs sm:text-sm font-medium text-slate-500">
        ...
        {/* Animated High-Fidelity Studio UI Showcase */}
        <div className="mt-14 sm:mt-18 relative max-w-5xl mx-auto">
          
          {/* Outer Glassmorphism Card Frame with Subtle Glow */}
          <div className="relative rounded-3xl p-2.5 sm:p-3.5 bg-gradient-to-b from-slate-200/90 via-slate-100/60 to-slate-200/90 shadow-2xl border border-white/90">
            
            {/* Inner 3D Viewport Simulation Frame */}
            <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-[#D2DAE8] via-[#DEE5F2] to-[#CBD5E6] aspect-[16/9] sm:aspect-[16/10] border border-slate-300/60 shadow-inner flex flex-col justify-between p-4 sm:p-6">
              ...
              <div className="flex items-center justify-between z-10">
                ...
                <div className="mx-auto bg-white/95 backdrop-blur-md rounded-full shadow-md border border-slate-200/80 px-3 py-1.5 flex items-center gap-2 text-xs font-semibold">
                  <span className="px-3 py-1 rounded-full bg-blue-600 text-white shadow-xs">Both Arches</span>
                  <span className="text-slate-600 px-2 hover:text-slate-900 cursor-pointer">Upper Only</span>
                  <span className="text-slate-600 px-2 hover:text-slate-900 cursor-pointer">Lower Only</span>
                  <span className="text-slate-600 px-2 hover:text-slate-900 cursor-pointer">Split View</span>
                </div>

                {/* View Cube Gizmo Mock */}
                <div className="absolute top-4 right-4 sm:right-6 w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/90 shadow-gizmo border border-slate-200/80 flex items-center justify-center animate-pulse-glow">
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-blue-600 text-white text-[10px] sm:text-xs font-bold flex items-center justify-center shadow">F</div>
                </div>
              </div>

              {/* Center Dental Arch Graphic Illustration with Dynamic Stage Simulation */}
              <div className="relative flex-1 flex items-center justify-center my-2 select-none">
                
                {/* Live Laser Caliper Ray Overlay (Sub-Millimeter Measuring Animation) */}
                <div className="absolute z-20 flex flex-col items-center pointer-events-none transform -translate-y-4">
                  <div className="flex items-center gap-1">
                    ...
                    <div className="h-0.5 w-24 sm:w-36 bg-blue-600 relative overflow-hidden">
                      <div className="absolute inset-0 bg-white/80 animate-scan" />
                    </div>
                    <div className="w-2.5 h-2.5 rounded-full bg-blue-600 ring-4 ring-blue-400/40 animate-ping" />
                  </div>
                  <span className="mt-1 px-2 py-0.5 rounded-md bg-blue-600 text-white text-[10px] font-mono font-bold shadow-md">
                    34.28 mm
                  </span>
                </div>

                {/* Visual Dental Arch SVG */}
              ...
              {/* Bottom Playback Timeline Bar with Live Animated Scrubber */}
              <div className="bg-white/95 backdrop-blur-md rounded-xl p-2.5 sm:p-3 shadow-md border border-slate-200/80 flex items-center justify-between gap-4 text-xs z-10">
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center shadow-sm transition-all"
                  >
                ...
                <div className="flex-1 flex flex-col gap-1 max-w-md">
                  <div className="flex justify-between text-[10px] font-bold text-slate-700">
                    <span className="text-blue-600 font-extrabold">{activeStage.text}</span>
          ...
          {/* Floating Feature Badges with Float Keyframe Physics */}
