...
export const ViewCubeGizmo: React.FC = () => {
  const { snapCameraTo, cameraTargetView, studioTheme } = useViewerStore();
  ...
  return (
    <div className="absolute top-2.5 sm:top-4 right-2.5 sm:right-4 z-20 select-none">
      <div className={`relative w-14 h-14 sm:w-20 sm:h-20 rounded-full backdrop-blur-md shadow-gizmo border p-0.5 sm:p-1 flex items-center justify-center transition-all hover:shadow-lg ${
        isDark
        ...
        <button
          ...
          className={`w-5 h-5 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold shadow-sm transition-all ${
            isFront
              ? 'bg-blue-600 text-white ring-1 sm:ring-2 ring-blue-400'
              : isDark
              ? 'bg-slate-800 text-slate-200 hover:bg-slate-700 hover:text-blue-400'
